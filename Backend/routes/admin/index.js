...
export default async function(fastify, opts) {
  ...
  fastify.get('/conferences', { schema: ConferencesSchema, onRequest: onRequest },
  ...
  fastify.get('/configs', { schema: ConfigSchema, onRequest: onRequest }, async function(request, reply) {
    await fastify.mssql.pool.connect();
    const res = await fastify.mssql.pool.query('SELECT * FROM users');
    return { users: res.recordset };
  })

  fastify.post('/configs', { schema: ConfigSchema, onRequest }, async function(request, reply) {
    await fastify.mssql.pool.connect();
    const res = await fastify.mssql.pool.query('SELECT * FROM users');
    return { users: res.recordset };
  })

  fastify.put('/configs/:id', { schema: ConfigSchema, onRequest }, async function(request, reply) {
    await fastify.mssql.pool.connect();
    const res = await fastify.mssql.pool.query('SELECT * FROM users');
    return { users: res.recordset };
  })
