...
function getToken(user) {
  const token = fastify.jwt.sign({ id: user.id, email: user.email, role: user.role }, { expiresIn: "1d" })
  return `Bearer ${token}`;
}
...
export default async function(fastify, opts) {
  ...
  fastify.post('/signin', { schema: AuthBodySchema }, async function(request, reply) {
    ...
    const match = await bcrypt.compare(request.body.password, user.hashed_password)
    if (!match) {
      throw fastify.httpErrors.unauthorized();
    }
  ...
  fastify.post('/signup', { schema: SignupSchema }, async function(request, reply) {
    const pool = await fastify.mssql.pool.connect();
  ...
  fastify.post('/password', { schema: ResetPasswordSchema, onRequest: [fastify.authenticate] }, async function(request, reply) {
    const pool = await fastify.mssql.pool.connect();
    let res = await pool
      .request()
    ...
    const user = res.recordset[0];
    if (!(await verifyPassword(request.body.password, user.hashed_password))) {
      throw fastify.httpErrors.forbidden();
    }
